...
jsou vyuity pøi tvorbì generátoru akcí v umìlé inteligenci.
...
Warlight.
- **Umìlá inteligence** - ve druhé kapitole zanalyzujeme problém tvorby umìlé inteligence do této hry a vybereme vhodnı algoritmus. Zamìøíme se na obecnì pouitı algoritmus a jeho modifikace pøizpùsobené znalostem této hry.
...
Cílem této kapitoly je popsat pravidla hry Warlight s pouitım nastavením.
...
Hráè z této mnoiny zvolí 2 regiony. Tato území pøedstavují vıchozí body, ze kterıch bude obsazovat další.

### Prùbìh hry
Hra se dìlí na herní kola, ty se dále dìlí na herní tahy. Kadé herní kolo
se skládá z tahù hráèù, kde kadı hráè pøispívá do kola právì jedním tahem.
...
Deploy fáze se skládá z deploy akcí. Pokud v jedné deploy fázi je více deploy akcí stavìjící jednotky na stejnı region,
...
na region sousední. Attack fáze se skládá z attack akcí. Pokud v jedné attack fázi je více attack akcí takovıch,
...
Po tomto potvrzení ji není moné je vrátit a jeho tah je povaován za uzavøenı.

### Kolo
Kadı hráè pøispívá do kola právì jedním tahem. Jakmile všichni hráèi ukonèí své tahy commitem, spustí se vıpoèet kola, kterı aktualizuje herní
stav.
...
- pokud pøi útoku pøeily bránící i útoèící jednotky, zbytek útoèících jednotek se po útoku vrátí na region, ze kterého pøišly
